...
export default function ProductListeOfOunCategorie({ navigation }) {
  ...
  const { categoryid } = route.params; // Get the productId from the route parameters
  console.log(categoryid)
  const handlenext = async () => {
    navigation.navigate('AddProductScreen');
  };

  const handleGoToAddIngredientWithID = async (product) => {
    navigation.navigate('AddIngredient', { product });
                ...
                <View style={styles.categoryContainer}>
                  {filteredProducts.map((Product) => (
                    <TouchableOpacity
                      key={Product._id}
                      style={styles.card}
                      onPress={() => handleGoToAddIngredientWithID(Product._id)}
                    >
                      <ImageBackground
